...
Accordingly, this script attempts to match PC vectors in my shuffled (and hence, unlabeled) training set to
the labeled values in the original PC dataset, whose rows are tied to RVE labels by a dictionary
in the data.py script.

In the future, you should just keep track of the shuffling by saving randomly permuted indices. 
This is probably way simpler.

Also, I re-wrote this code to be cleaner (can you believe it was worse than this before?). It *should* work.
Again, the easier solution is to randomly permute indices in the construction of the test-train split. And not use
this file at all.
"""
